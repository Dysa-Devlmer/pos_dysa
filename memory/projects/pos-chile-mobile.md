...
**G-M44 (regla canónica audit mobile sin device — 2026-04-28)** — **Antes de teorizar runtime bugs UX en mobile, verificar empíricamente que el bundle Metro compila.** El audit 2026-04-28 produjo inicialmente una "Adenda V: SS1-SS8 = 6-8 bugs runtime simultáneos en APK" leyendo solo los síntomas reportados por el usuario ("no entra al login, no se ven colores, no tiene diseño"). Ese modelo era plausible viendo el síntoma pero salteaba el paso elemental: en realidad **el bundle Metro NO compilaba** (causa real documentada en G-M43). Sin bundle, todos los síntomas son la **misma falla** manifestándose, no N bugs paralelos. Cowork lo confirmó empíricamente conectando un Xiaomi Redmi Note 14 (Android 15) por USB debugging + ADB y cerrándolo en `225c7b5`. **Regla obligatoria para futuros audits mobile cuando el agente NO tiene device físico**: exigir al usuario una de estas dos evidencias antes de generar cualquier plan de fixes runtime — (a) screenshot de Metro corriendo con un `Android Bundled XXXXms / NNNN modules` en logs, o (b) `adb logcat` filtrado por la app mostrando que el JS context arrancó. Sin esa evidencia, cualquier teorización SS1-SSN es especulación cara que confunde el plan y consume ciclos del CEO/Worktree corrigiendo premisas. SS1-SS6 quedan como **verificaciones post-bundle a hacer en device cuando se llegue al Día 2**, NO como fixes simultáneos asumidos: SS5 (DefaultTheme blanco React Navigation) probablemente parcialmente resuelto ya por `darkMode: "class"` del fix M0; SS7 (Network Security Config) y SS8 (NativeWind+Hermes+new-arch edge) bajan de "hipótesis activa" a "verificar solo si reaparece síntoma en QA". El "Unmatched Route" que aparece al testear con `adb shell am start --/` NO es bug del componente — es artifact del modo CLI de Expo Go interpretando `--/` como path literal; en APK release o tap normal el `<Redirect>` funciona.
...
**G-M45 (jest-expo + pnpm)** — Las deps reales en pnpm viven en `node_modules/.pnpm/<pkg>@<ver>_<hash>/node_modules/<pkg>/`. El `transformIgnorePatterns` default de jest-expo NO matchea ese path → `react-native/jest/setup.js` y `expo-modules-core/.../*.ts` fallan con `SyntaxError: Cannot use import statement outside a module`. Fix: prefix opcional `(?:.*?node_modules/)?` antes de la lista de paquetes + `[\\w-]*` para cubrir scopes (`expo-*`, `@expo/*`, `@react-native-*`). Patrón actual en `apps/mobile/jest.config.js` confirmado funcionando con 46 tests pasando.

**G-M46 (jest-expo 54 ↔ jest 30 incompatible)** — `jest-expo@54` peer dep wants `jest@^27 || ^28 || ^29`. Pin obligatorio `jest@^29.7.0` + `@types/jest@^29.5.14` o `jest-watch-typeahead` arroja peer warning + posibles fallos runtime. Verificado al instalar inicialmente jest 30 y obtener warning explícito.

**G-M47 (APK install MIUI Security)** — Aún con `Depuración USB` + `Instalar via USB` + `Depuración USB (Configuración de seguridad)` activos en MIUI, `adb install` retorna `INSTALL_FAILED_USER_RESTRICTED: Install canceled by user` (sin mostrar dialog en device). Workaround verificado: `adb push <apk> /sdcard/Download/` + abrir manualmente con Mi File Manager → Tap en el APK → Install. Probable causa: anti-fraude MIUI requiere SIM activa + cuenta Mi logueada en device para destrabar adb install. Documentado en sesión 2026-04-29 con APK dev build.

**G-M48 (dev build APK necesita Metro corriendo)** — APK generado con `expo run:android` NO embebe el bundle JS — descarga el bundle de Metro al arrancar. Si Metro no está up + `adb reverse tcp:8081 tcp:8081` no está activo, app queda eternamente en splash screen (ni siquiera muestra error claro). Diferencia clave con APK release/EAS Build (que sí embebe bundle). Para test en device sin Mac local: usar EAS Build remoto o `expo build --type apk` con bundle inline.
