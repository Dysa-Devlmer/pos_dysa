...
> **🆕 2026-04-28 — M0 BLOQUEO descubierto en hardware real:** Cowork verificó la app en Xiaomi Redmi Note 14 (Android 15, API 35) conectado por USB con Expo Go 54.0.6. **El bundle Metro NO compilaba** por bugs de versiones (NativeWind 4.2.3 + css-interop@0.2.3 emiten `@cssInterop set` que su propio parser rompe; `important: "html"` en tailwind config genera selectores que también rompen el parser; default Node 18 no soporta SDK 54 Metro). Fixes aplicados (sin commitear aún): `.nvmrc=20.19.5`, pin `nativewind=4.1.23` exacto, removido `important: "html"` de tailwind config, 7 packages SDK 54 alineados manualmente. **Plan M0** documentado como sección Ñ del `reporte.md` (nuevo bloqueante #2 en TL;DR). Smoke test en device pendiente — Metro arranca pero el device no completó el primer bundle request. Ver gotchas G-M39, G-M40, G-M41 (obsoleta G-M36), G-M42.
